...
#ifdef __Arthur_Wang__
    if (NULL != overlay->rgbData) {
        free(overlay->rgbData);
        overlay->rgbData = NULL;
    }
#endif // __Arthur_Wang__
...
static int func_fill_frame(SDL_VoutOverlay *overlay, const AVFrame *frame)
{
    ...
    overlay->pixels[0]  = NULL;
    overlay->pixels[1]  = NULL;
    overlay->pitches[0] = CVPixelBufferGetWidthOfPlane(pixel_buffer, 0) * 2;
    overlay->pitches[1] = CVPixelBufferGetWidthOfPlane(pixel_buffer, 1);

    int RGB565Width = frame->width;
    int RGB565Height = frame->height;

    overlay->format = SDL_FCC_RV16;
    if (NULL == overlay->rgbData) {
        overlay->rgbData = malloc(RGB565Width * RGB565Height * 2);
    }

    UInt8 *YPtr = (UInt8 *)CVPixelBufferGetBaseAddressOfPlane(pixel_buffer,0);
    UInt8 *UVPtr1 = (UInt8 *)CVPixelBufferGetBaseAddressOfPlane(pixel_buffer,1);

    NV12ToRGB565(YPtr, (int)((frame->width + 63) & ~63),
                 UVPtr1, (int)((frame->width + 63) & ~63),
                 overlay->rgbData, RGB565Width * 2, RGB565Width, RGB565Height);

    overlay->pixels[0] = overlay->rgbData;
    CVPixelBufferUnlockBaseAddress(pixel_buffer, 0);

    overlay->is_private = 1;

    overlay->w = RGB565Width;
    overlay->h = RGB565Height;
